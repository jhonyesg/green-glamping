...
async def simulate(
    ...
    return JSONResponse({
        ...
        "llm_calls": 1 if any(s["step"] == "llm_decision" and (s["detail"] or {}).get("decision") == "llm_invoked" for s in trace) else 0,
        "total_ms": total_ms,
        "intent": result.outbound.intent_name or "fallback",
        "response_preview": (result.outbound.text or "")[:300],
        "response_text": result.outbound.text or "",
        "media_attachments": result.outbound.media_attachments or [],
        "requires_human": result.outbound.requires_human or False,
        "handoff_rule": result.outbound.handoff_rule or None,
        "trace": trace,
